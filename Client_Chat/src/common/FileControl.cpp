...
bool FileControl::readFile(QString path, QJsonArray &data)
{
    QFile file(path);
    if(!file.open(QIODevice::ReadOnly | QIODevice::Text))
    {
    ...
    QString str = file.readAll();
    data = QJsonDocument::fromJson(str.toUtf8()).array();
    file.close();
    return true;
}
...
bool FileControl::writeFileJson(QString path, QJsonArray &data)
{
    QFile file(path);
    if(!file.open(QIODevice::WriteOnly | QIODevice::Text))
    {
